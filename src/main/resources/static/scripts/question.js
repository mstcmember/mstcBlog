...
function onloadHotQuestion() {
    ...
    $.ajax({
        ...
        success: function (data1) {
            ...
            {
             var obj_lis = document.getElementById("hotQuestion_content").getElementsByTagName("a");
                 var random=[];
                 ...
                 for (i = 0; i < obj_lis.length; i++) {
                    ...
                    obj_lis[i].href = "/questionDetail.html?questionId=" + blogHotList[b].id;
                 }
            }
...
function aquireQuestionComment(start) {   //获取评论（从第start条开始向后获取10条）
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if($.isEmptyObject(user)){
                temp.hostHolderId = 0;
                temp.hostHolderFlag = 0;
            }else {
                temp.hostHolderId = user.id;
                temp.hostHolderFlag = user.flag;
            }
...
//删除技术帖
function deleteQuestion(type,id,owerId){
if(confirm("确认删除？")){
    var json = {};
